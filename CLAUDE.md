...
1. **驗證 V3.4.19 C 系列大擴張** — 部署後第一件事，確認 (a) sidebar「C 各癌別照護指引」展開後看到 14 項（C0 + C1-C13）(b) C7-C13 七個新檔內容完整（每檔 8 段、有準則引用、有「待醫師團隊確認」警示）(c) C4↔C9、C6↔C10 cross-ref 連結可點 (d) C3-C6_other-cancers.md 已不存在（該檔的 4 癌別內容散到獨立檔）

**重要的下一步：**

2. **C7-C13 內容請醫師團隊審閱**（最重要）— V3.4.19 補的 7 個新檔是依公開準則整理的草稿，明確標明「⚠️ 待醫師團隊確認」。優先審：
   - C7 食道癌：CCRT 三合一療法 + 健保 2024/4 二線免疫給付細節
   - C8 胃癌：HER2 / MSI / PD-L1 三標記分流 + 健保 2024/4 一線免疫給付（PD-L1 CPS≥5）
   - C9 肝膽胰：Whipple 術後照護、mFOLFIRINOX 副作用、PARP 抑制劑使用
   - C10 泌尿：BCG 灌注、尿路重建（迴腸通道/新膀胱）
   - C11 婦癌：CCRT（子宮頸）+ BRCA 檢測（卵巢）+ FIGO 2018/2023 分期
   - C12 血液淋巴：CAR-T 適用條件 + 自體幹細胞移植 + Stupp 方案
   - C13 其他癌：通用原則是否適合本院運作

3. **C3-C6 之前的醫師審閱**（V3.4.18 已補的內容）也要審：mCRC 免疫健保 2025/6 給付、BCLC 分期建議、頭頸 CCRT、攝護腺風險分層

**等醫師團隊提供**

4. C1 肺癌剩餘 placeholder（流行病學年齡等）
5. F 系列治療概念引用準則版本
...
> **C 系列癌別覆蓋已完整**：12 個癌別檔 + C0 通論 = 涵蓋 A/B/C 三位個管師所有負責癌別。剩下主要是醫師審稿回饋整合。

---

## 八、一句話總結

V3.4.19：**C 系列大擴張**。V3.4.18 後重跑新人 28 天試用發現結構破口——A1 分工表列 10 個 MDT 癌別，但 C 系列檔只覆蓋 6 個（C0 通論+C1 肺癌+C2 乳癌+C3-C6 大腸/肝/頭頸/攝護腺），缺食道癌、其他癌、胃癌、血液淋巴癌、婦癌、膽道癌、胰臟癌、腎癌、膀胱癌共至少 9 個癌別沒有專屬內容。新人 W4 跟學姊收案時遇到負責的癌別，只能在 G/F4/H3 拼湊片段。**Sela 選方案 A 補新檔**。**動了三步**：(1) 拆 C3-C6_other-cancers.md 為 4 個獨立檔（沿用 V3.4.18 內容）+ C4↔C9、C6↔C10 cross-ref (2) 新建 C7-C13 七檔，依公開準則整理（NCCN 2025、健保署 2024/2025 給付公告、台灣相關醫學會），每檔固定 8 段架構、含具體治療藥物名稱（FOLFIRINOX、CAR-T、Stupp 方案、PARP 抑制劑等）+ 4-5 個 Q&A，每檔標明「⚠️ 待醫師團隊確認版本與本院實作差異」(3) mkdocs.yml nav 重組為 14 項。**順便修問題 2**：A1 第一部分標題後多餘空白行（V3.4.14 H4→H3 升級時殘留）。C 系列總行數從 1280 → 3867 行（3 倍）。**踩到的小坑**：grep 中文 regex `^### [一二三四五六七八]、` 抓不到、改用最簡單 `^### ` 驗證。**接手第一件事**：Sela 部署實測 + **排時間請醫師團隊優先審閱 C7-C13 七個新檔內容**。**結構性破口已堵**——個管師 A/B/C 各自的 3-4 個負責癌別現在都有專屬照護指引。下一階段主軸是醫師審稿回饋整合。
