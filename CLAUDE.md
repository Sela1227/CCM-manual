...
| V3.4.18 | **新人 28 天時間軸試用 + C3-C6 補臨床知識**：清 V3.4.15 漏網 17 處 placeholder（B1+B2+C1+E 散在）+ A1 結業測驗加說明 + A1 課程表加 cross-ref 自學參考 + **C3-C6 八段內容依公開準則補臨床細節（從 238→927 行，4 倍）**，標明「待醫師團隊確認」（坑 #42）|
| V3.4.19 | **C 系列癌別涵蓋不全 → 補 7 個新檔（方案 A）**：拆 C3-C6 為 4 獨立檔 + 新建 C7-C13 七個（食道/胃/肝膽胰/泌尿/婦癌/血液淋巴/其他癌）**全部依公開準則整理 + 標明「待醫師團隊確認」**。C 系列從 6 → 12 癌別+通論。順便修 A1 第一部分多餘空白行（坑 #43）|
| V3.4.20 | **logo.png 砍除 + 第四輪 QA**：砍 logo.png（V3.4.1 起無引用、244 KB）規則 11 簡化為 2 logo 檔；第四輪 QA 三視角組合試用發現 1 必修（首頁「各癌別」卡片改連 C0 通論）+ 2 觀察。前三輪 QA 累積修補品質高（坑 #44）|
...
| **V3.4.22** | **美編+UI 視角審視 7 件修補**：(1) CSS 開頭註解清理（清歷史描述只留版本標記）(2) 首頁副標砍除 (3) 卡片描述字級 0.81→0.9rem (4) 手機底部 3 鈕標籤改更直覺（藥/品質/臨床）(5) 表格 header 改淺灰底+主色字+主色底線（降視覺重量）(6) H3 加細左線+字級 (7) 版本徽章從 header 移到 footer 右下角 fixed。**踩到坑：sed 升版時誤改 JS 註解**內的版本號→啟示「歷史註解不該寫死版本號」（坑 #46）|
...
1. **驗證 V3.4.22 美編修補** — 部署後第一件事，確認 (a) 首頁無副標、卡片描述字級略大 (b) 手機底部標籤是「藥/品質/臨床」(c) 表格 header 是淺灰底+深藍字（不再是深藍實心）(d) H3 標題前有細左線 (e) 版本徽章從 header 消失、出現在右下角小徽章 (f) `extra.css` 開頭註解只有版本標記沒歷史描述
2. **規則 1 修訂**：歷史註解不應該包含版本號（避免 sed 升版誤改）。修法：開新規則「版本號只出現在 4 處 + 歷史/註解用『本版』『上一版』中性敘述」
...
12. 美編+UI 第二輪審視（這次主要動 CSS + 首頁 + 手機底部，內文檔的字級/間距/表格密度等還沒審）
13. emoji 用法一致化工程（觀察 #8，工程大）
14. 再做一次 QA 試用

> **美編+UI 視角第一輪完成**——主要動 CSS 結構（表格、H3、首頁字級）+ 版本徽章位置 + 手機標籤。下次可由醫師審稿回饋觸發、或 Sela 主動發起其他視角審視

---

## 八、一句話總結

V3.4.22：**美編+UI 視角審視 7 件修補**。Sela 切換成「專業美編 + 系統設計師」雙重角色，找視覺美感及 UI 缺陷。**最重要發現是表格 header 視覺過重**：原本主色 `#25506b` 實心 + 白字，在 D1（633 行 20+ 表）這種表多的檔整頁飽和。改成淺灰底 `#eef2f5` + 主色字 + 主色底線，保留辨識度但降低視覺重量。**首頁卡片描述字級 0.81rem ≈ 13px 太小**（醫療使用者年齡偏高），改成 0.9rem。**手機底部標籤辨識弱**：「藥物/指標/知識」改「藥/品質/臨床」更直覺。**版本徽章從 header 移到 footer 右下角**：原本掛在站名旁像「測試版」，改 fixed position + 透明度 0.7，手機 viewport 閃避底部 6 鈕導覽列。**首頁副標砍除**（主標題已夠清楚）、**H3 加細左線+字級**（補強層級對比）、**CSS 開頭註解清理**（歷史描述移除避免誤導）。**踩到的坑**：升版號 sed 把 JS 註解內剛寫的「V3.4.22 從 header 移到 footer」一併改了→ 註解裡寫死版本號是 anti-pattern、會被 sed 誤動。**規則 1 需修訂**：版本號只該出現在 4 處（CCM_VERSION + CSS 開頭版本標記 + README + CLAUDE.md 頂部），註解用「本版」「上一版」這類中性敘述。**接手第一件事**：Sela 部署實測 (a) 首頁無副標卡片字級略大 (b) 手機底部新標籤 (c) 表格淺底 (d) 版本徽章在右下角。**下一階段**：等醫師審稿回饋 / 或 Sela 發起其他視角審視（美編 / PM / QA 第二輪）。
