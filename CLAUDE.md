...
| V3.4.11 | **規則 13 拆檔第一輪**：F 檔（1302 行）拆 5 子檔做巢狀（F1 檢驗 / F2 分子病理 / F3 影像 / F4 治療 / F5 追蹤），主檔變導覽頁；D 檔盤點發現 D2/D3/D4 各有兩版重複需先合併，這版未動列下版（坑 #35）|
...
| **V3.4.14** | **新人視角 QA 修補**：從新進個管師角度試用發現 10 個問題、修 6 個——首頁架構表更新（B+H 合併）、F 子檔 5 個引言重複砍除、A1「個管師是什麼角色？」+「什麼時候要轉介？」H4 升 H3、D1/D4 重複 `---` 砍除、B1 補 H 系列段。剩 4 個等資料/不修（坑 #38）|
...
1. **驗證 V3.4.14 新人視角修補** — 部署後第一件事，確認 (a) 首頁「本指導書架構」表有「軟體與工具」一行（不是舊的 B+H）(b) F 子檔（F1-F5）開頭只有一個引言段（不重複）(c) A1 TOC 有「個管師是什麼角色？」「什麼時候要轉介？」兩個 H3 (d) D1/D4 開頭乾淨無重複 `---` (e) B1 第 1 章「子系統總覽」之後有「癌症中心專用軟體（不在 HIS 內）」段，含 H1-H4 連結
2. **驗證 V3.4.13 A2 體驗微調**（如未實測）— sidebar 看「A2 培訓計畫書（制度文件）」副標 + cross-ref 9 處可點擊
3. **驗證 V3.4.10-V3.4.12 各版本**（如未實測）

**等資料類**（V3.4.14 試用發現的問題 5/6/7）

4. **補 B1 各 HIS 子系統的操作截圖**（問題 5）— 12 個 HIS 子系統「待補充截圖」「待補充操作步驟」
5. **B1 第 3 章預設密碼**（問題 6）— 「待填」可改成「請問人事室／資訊室」當過渡
6. **A1 附錄聯絡方式**（問題 7）— 「直屬主管／資深學姊／癌症中心辦公室／資訊室／放腫科林醫師門診」全是「（待填）」

**等資料類**（先前候選）

7. 抗癌藥物速查若有新版（整個 drug-lookup/ 替換 + 改 H3 版本說明，規則 9）
8. 子宮頸癌 / 子宮體癌 / 卵巢癌完治率定義補入 G_quality-index.md
9. NGS 內容後續更新（同步改 F2_pathology.md NGS 段表格）

**雜項**

10. **logo.png（首頁大 logo 600px）是否還需要？** V3.4.1 起所有引用都移除了
11. **再做一次新人視角試用** — V3.4.14 修了 6 個結構/內容問題，未來實際資料填入後可再做一次 QA、找新一輪問題

> 結構工程（規則 12 + 13）+ 新人視角 QA 都告一段落。剩下都是內容補充類，等實際資料才能動

---

## 八、一句話總結

V3.4.14：**新人視角 QA 修補**。Sela 提議「從新進個管師的角色試用系統」找出 10 個問題：4 個結構/內容 bug（首頁架構表過時、F 子檔引言重複、A1 兩個 H3 該升的沒升）、2 個小工程（D1/D4 多餘 `---`、B1 沒列 H 系列）、3 個等資料（B1 截圖、密碼、A1 附錄聯絡方式）、1 個不修（A2 cross-ref anchor 精準度，slug 不穩定）。修了 6 個（4 必修 + 2 順手），其他列候選清單。**踩到的小坑**：(1) str_replace 改 H4→H3 時 old/new 寫一樣沒升級，改用 sed 解決——教訓 str_replace 改層級時 old/new 要明確不一樣 (2) F 子檔引言重複根因在 V3.4.11 的 split_F_v3411.py，但只修結果不重寫腳本（一次性遷移 bug，不必修腳本）。**接手第一件事**：Sela 部署實測 (a) 首頁架構表正確 (b) F 子檔引言不重複 (c) A1 兩個新 H3 在 TOC (d) D1/D4 開頭乾淨 (e) B1 補的 H 系列段。**下一階段建議**：等實際資料（截圖、藥物表更新、完治率定義、NGS）才能動；資料填入後可再做一次新人視角 QA 找新一輪問題。**規則 12 + 13 + QA 三大階段全部告一段落**，從這版起進入純內容補充期。
