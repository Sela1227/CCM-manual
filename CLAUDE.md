...
1. **驗證 V3.4.1 三項修補視覺** — 部署後第一件事，確認 (a) index.md 首頁不再有 SELA 大圖、只剩標題（hero 區塊還在但只有 h1）(b) 行動裝置開 drawer 邊欄不再有 SELA 橘色 icon（站名仍在）(c) 內文左右有 padding 不貼邊框（截圖 2 那種貼邊框情況不再出現）(d) C0 通論〈Stage IV ≠ 一定轉移〉新章節有正常渲染、AJCC 表格與其他細分癌別表格都在
...
7. **logo.png 是否還需要？** V3.4.1 把 logo.png（首頁大 logo 600px）的所有引用都移除了，但檔案還留在 docs/assets/。下次清理時可考慮砍——但不急，留著不會壞 build；規則 11 仍要求三檔在但 logo.png 沒被引用，必要時可從規則 11 移除這條

---

## 八、一句話總結

V3.4.1：V3.4.0 部署後 Sela 反映三件事——logo 散在 header / 內文 / drawer 邊欄三處應該收斂、內文左邊太貼邊框讀不下去、C0 通論「Stage IV ＝ 遠端轉移」一刀切會誤導頭頸癌病人。這版就是把這三件事一次解掉：logo 只留 header（index.md hero 砍 img、CSS 隱藏 drawer logo）、`.md-content__inner` 加 1/1.5rem padding、C0 通論修主表格 + tip 框 + 新增〈Stage IV ≠ 一定轉移〉子章節含頭頸癌 AJCC 亞期表。CSS 仍維持 V3.2.0 乾淨基底（規則 8 不破）。**接手第一件事**：Sela 部署實測三項修補有沒有到位，特別是 (1) drawer 邊欄那個 SELA 橘色 icon 真的消失了（不是只剩半個變奇怪）(2) 內文 padding 在 wide / tablet / mobile 各 viewport 都正常 (3) C0 通論的新表格沒翻車。
