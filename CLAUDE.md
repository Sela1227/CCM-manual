...
1. **驗證 V3.4.13 A2 體驗微調** — 部署後第一件事，確認 (a) sidebar 看到「A2 培訓計畫書（制度文件）」副標 (b) A2 內 cross-ref 段（檔頭 4 個 + 第一章引言 + 第二章引言 + 第三章引言 + 第四章兩個）9 處連結都能點擊跳轉
2. **驗證 V3.4.12 D 檔合併拆檔**（如未實測）— D 巢狀組 5 項、第二版獨家內容
3. **驗證 V3.4.11 F 檔拆檔**（如未實測）— F 巢狀組 6 項
4. **驗證 V3.4.10 A2 重整**（如未實測）— A2 4 章 + 附錄結構
...
9. NGS 內容後續更新時注意：健保給付癌別會擴增（目前 19 種是 113 年 5 月版），補入時記得同步改 F2_pathology.md NGS 段表格

> **規則 12（標題層級設計語言）+ 規則 13（拆檔巢狀門檻）兩條結構規則均已套用完畢**：規則 12 修了 A/C3-C6/G 三檔（F 重新評估後不違規）；規則 13 修了 A/B/C/H/D/F 共 6 大組巢狀化，E/G 因不到門檻保留單檔。**剩下的候選工作主要是內容補充類（截圖、藥物表更新、完治率定義、NGS 更新）**，不再是結構重整

---

## 八、一句話總結

V3.4.13：**體驗微調版本**——把 V3.4.10 留下的兩個衍生待辦做完。**A2 nav 加副標**「（制度文件）」用方案 Y（最小改動，只改 mkdocs.yml 一行），讓 sidebar 一眼看到 A2 定位跟 A1（工作手冊）不同；不破現有結構、不動 A1、不動其他檔。**cross-ref 9 處 markdown 化**：A2 檔頭 4 個 + 第一/二/三章引言段 + 第四章兩個總共 9 處，從粗體文字 `**A1 工作指導手冊**` 改成可點擊連結 `[**A1 工作指導手冊**](A_work-guide.md)`，保留視覺粗體 + 加跳轉功能。文件資訊段的「請查 A1/B1/F/G」是說明性質不動。純 str_replace 6 次 + 改 nav 一行，mkdocs --strict 0 warning。**踩到的小坑**：grep 驗證 cross-ref 改完寬鬆模式 `^> .*\*\*[ABCDFGH][0-9]* ` 會誤抓「已包在 markdown 連結內的粗體」，要人工複查確認都已 markdown 化。**接手第一件事**：Sela 部署實測 (a) sidebar 看「A2 培訓計畫書（制度文件）」副標 (b) cross-ref 9 處可點擊。**規則 12+13 結構工程全部告一段落**，候選工作從這版起進入「內容補充期」（補 B1 截圖、藥物表更新、完治率定義、NGS 更新等）。
