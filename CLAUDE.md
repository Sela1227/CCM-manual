...
1. **驗證 V4.0.0 對齊 Kit 落實** — **本版重點交付物的可用性確認**。檢查 (a) CLAUDE.md 開頭有 Kit 衝突仲裁區塊 (b) SELA-handoff.md 存在 + 內容對齊 Kit template 七章節 (c) .gitignore 含 Kit 模板項目 (d) README.md 有 Kit 對齊聲明 (e) 規則 16 已建立、坑 #53 是首個三段式範例 (f) `mkdocs build --strict` 0 warning

**最重要的下一步：handoff 跨對話接力**

2. **下次升 Kit 時把 SELA-handoff.md 帶過去**——不再丟整份 CLAUDE.md。Kit Claude 看 handoff 就能高效規畫升 Kit 內容（建議坑 #46-#48 + 章法十一時效性 + Single Source of Truth 進 Kit）

**剩餘 placeholder（A3 + 環境章節 + 五癌篩檢承辦人，同批詢問主管）**

3. **A3 剩餘 placeholder**（V3.4.25）：38 醫師分機 / 個管師真名 / 共通支援單位 / 行政秘書 / 「人文會議室 vs 人文藝術館會議室」
4. **V3.4.23 環境章節 placeholder**
5. **V3.4.27 五癌篩檢推廣承辦人**

**最重要的內容下一步（C 系列審稿）**

6. **C7-C13 七個新檔請醫師團隊審閱**（V3.4.19 補的）—— 現在有 A3 醫師清單 + G2 實際成績可對應
7. **C3-C6（V3.4.18 已補的）也要醫師審**

**等醫師團隊**

8. C1 肺癌剩餘 placeholder
9. F 系列治療概念引用準則版本
10. H2 MDT 系統「個案資料最少必填欄位」

**老手視角剩餘**

11. **電話追蹤內容三處整合（D3/F5/A1，規則 14 待套用）**

**等資料類**

12. 補 B1 各 HIS 子系統的操作截圖
13. 抗癌藥物速查若有新版
14. NGS 內容後續更新
15. **時效性內容年度檢視**（規則 15）：每年檢視 EGFR / 五癌篩檢 / 健保藥物 / **G2 替換為 115 年度版**
16. 五癌篩檢推廣 QR code 2027 年改版

**雜項**

17. 美編+UI 第二輪審視（內文檔字級/間距/表格密度等）
18. emoji 用法一致化（觀察 #8，工程大）
19. 再做一次 QA 試用（醫師審稿回饋整合後）

---

## 八、一句話總結

V4.0.0 **首次對齊 SELA Kit V1.9.0**：7 件做（衝突仲裁區塊 + .gitignore 補強 + README 對齊聲明 + 下版候選 #1 加解釋 + 一句話總結縮短 + 規則 16 三段式 + 首份 handoff）；5 件不做明寫理由（SELA logo + 配色 + 既有 52 坑格式 + 章法手冊 4 個不適用段 + 既有 15 條規則）。**規則 1 補充澄清**：版本號 4 處 + 歷史錨點是合法例外。**下版第一優先**：驗證 V4.0.0 對齊 Kit 落實——本版重點交付物的可用性確認。
