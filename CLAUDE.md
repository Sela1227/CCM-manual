...
1. **驗證 V3.4.8 C3-C6 + G 重整** — 部署後第一件事，確認 (a) C3-C6 在 sidebar/TOC 是 1 H1 + 4 H2（4 癌別）+ 12 H3（每癌別 3 子段並列展開）(b) G 是 1 H1 + 7 H2 + 19 H3 + 11 H4，13 癌別在「各癌別指標詳表」之下整齊排列、9 個完治率癌別在「個案管理三大追蹤指標」之下排為 H4
2. **驗證 V3.4.7 A 檔層級重排視覺**（如未實測）— A 檔 TOC 32 項、5 主題塊收攏、邊緣 7 個 H4 在內文是否清楚
3. **B/H 整合在「軟體與工具」一區（Sela V3.4.7 提）** — B（B1 HIS、B2 其他工具）跟 H（H1-H4 癌症中心專用）**都是「講軟體的」**，現在分散在 nav 兩處（B 在第 2 區、H 在最後一區）。整合方案待討論：(a) 真合併檔名 B1→S1...H4→S6（大工程，要改 mkdocs.yml nav、首頁卡片連結、手機底部導覽連結）(b) 不改檔名只改 nav 視覺位置（讓 B 跟 H 在 sidebar 相鄰）(c) 加「軟體與工具總覽」入口頁當索引。等 Sela 確認方向再動。**注意**：B 是醫院共用 HIS（外部供應商）vs H 是癌症中心自開發 web app，整合後可能失去這層分類，要評估
4. **驗證 V3.4.5 + V3.4.6 內容重排視覺**（如未實測）— F 檔 F1–F5 五段、A1 是 4 個部分、B1 是 18 章
5. **驗證 V3.4.4 手機底部導覽 6 個鈕視覺**（如未實測）— 6 個鈕在 360px 寬手機都能塞下圖示+標籤
6. **A1 第二部分 vs A2 培訓計畫書是否重複（下一階段）** — A1「新人養成指引」（學習路線圖、四階段培訓、checklist）跟 A2 主題正面對撞。**等 Sela 主動提**
7. **A1 附錄是否要動（下一階段）** — A1 末尾「附錄」含「重要聯絡資訊」「相關文件索引」，內容是 placeholder（待填）。**等 Sela 主動提**
8. **B1 + F 檔的「## 目錄/章節架構」也要砍**（規則 12 第三條）— A 檔已砍，B1 跟 F 檔的「## 目錄/章節架構」也適用同規則。下次動 B1/F 檔時順手
9. 補 B1 各 HIS 子系統的操作截圖（V3.4.6 後 B1 集中為單一系統手冊，新人最需要）
10. **F 檔巢狀化（規則 13 該拆）** — F 檔 1302 行 > 1000 行門檻，按規則 13 該拆 5 檔。但因為 V3.4.5 已內容重整完、單檔仍可讀，**優先序低**，等 Sela 主動提
11. **D 檔巢狀化（規則 13 該拆）** — D 檔 1479 行 > 1000 行門檻，按規則 13 該拆。需先看 D1-D4 內部結構
12. 抗癌藥物速查若有新版，整個 `docs/drug-lookup/` 資料夾原地替換 + 改 H3 版本說明（規則 9）
13. 子宮頸癌 / 子宮體癌 / 卵巢癌完治率定義補入 G_quality-index.md（V3.4.8 重整後在「個案管理三大追蹤指標 → 各癌別完治率定義」H4 之下，待團隊討論）
14. **logo.png（首頁大 logo 600px）是否還需要？** V3.4.1 起所有引用都移除了，下次清理時可考慮砍
15. NGS 內容後續更新時注意：健保給付癌別會擴增（目前 19 種是 113 年 5 月版），補入時記得同步改 F2 NGS 段表格

---

## 八、一句話總結

V3.4.8：**規則 12 套用第二輪**——C3-C6 + G 兩個違規檔修完，F 檔重新評估後從違規清單拿掉。三件事：(1) **C3-C6**（模式 2 完全沒 H3）：4 個 H1（4 癌別）+ 12 H2 + 0 H3 → 1 H1（新加）+ 4 H2（4 癌別並列）+ 12 H3（每癌別 3 子段），4 癌別並列對稱、子段在 TOC 上展開 (2) **G**（模式 4 + 隱藏的「5 個 H1」問題）：5 H1 + 22 H2 + 11 H3 → 1 H1 + 7 H2 + 19 H3 + 11 H4，做法是 line 52 之前不動、之後全部降一級的「一刀切」重整 (3) **F 檔誤判修正**：6 H2 + 18 H3 + 40 H4 原本被列為「模式 3 違規」，重新評估後 TOC 24 項已合理、H4 多是內容性質（影像關鍵詞、藥物分類）不需上 TOC——符合規則 12「TOC 是快速到附近」原則，**F 檔不算違規**。**B/H 整合待辦**（Sela V3.4.7 提）加入候選 #3，等她確認方向再動。Python 切片 + 一刀切降級，mkdocs --strict 0 warning。**接手第一件事**：Sela 部署實測 (a) C3-C6 sidebar/TOC 看 4 癌別 + 12 H3 是否整齊 (b) G 看 7 H2 + 19 H3 結構是否好用 (c) B/H 整合方向決定。坑 #31 違規檔 4 個全部處理完（A V3.4.7 / C3-C6 V3.4.8 / G V3.4.8 / F 重評不違規），規則 12 套用告一段落。
