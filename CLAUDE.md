...
| **V3.4.15** | **placeholder 過渡寫法（V3.4.14 延伸）**：清 A1+B1 共 17 處「（待填）」改成行動指引（「請洽該單位」「報到時主管提供」「詳見 H1/D2」等），不再讓使用者卡在 placeholder（坑 #39）|
...
1. **驗證 V3.4.15 placeholder 過渡寫法** — 部署後第一件事，確認 (a) A1 院內資源表的分機欄都顯示「（請洽該單位）」(b) A1 附錄重要聯絡資訊欄都是「報到時主管提供」(c) A1 附錄相關文件索引各有精準指向（「請洽主管」「請洽資訊室」「詳見 H1」「詳見 D2」）(d) B1 第 3 章預設密碼是「請洽人事室或資訊室索取（首次登入後請立即修改）」
2. **驗證 V3.4.14 新人視角修補**（如未實測）— 首頁架構表、F 子檔引言、A1 兩個新 H3、D1/D4 開頭、B1 H 系列段
3. **驗證 V3.4.10-V3.4.13 各版本**（如未實測）

**等資料類**

4. **補 B1 各 HIS 子系統的操作截圖** — 12 個 HIS 子系統「待補充截圖」「待補充操作步驟」
5. 抗癌藥物速查若有新版（整個 drug-lookup/ 替換 + 改 H3 版本說明，規則 9）
6. 子宮頸癌 / 子宮體癌 / 卵巢癌完治率定義補入 G_quality-index.md
7. NGS 內容後續更新（同步改 F2_pathology.md NGS 段表格）

**雜項**

8. **logo.png（首頁大 logo 600px）是否還需要？** V3.4.1 起所有引用都移除了
9. **再做一次新人視角試用** — 實際資料填入後可再做一次 QA、找新一輪問題

> 結構工程（規則 12 + 13）+ 新人視角 QA + placeholder 過渡寫法都告一段落。剩下都是內容補充類，等實際資料才能動

---

## 八、一句話總結

V3.4.15：**V3.4.14 的延伸完成**——把 V3.4.14 列「等資料」的問題 4-6（B1 密碼、A1 附錄聯絡方式、A1 院內資源分機）用「過渡寫法」做掉。**「待填」placeholder 對使用者沒幫助、過渡寫法給方向**——「請洽人事室或資訊室索取」「報到時主管提供」「詳見 H1 個管追蹤系統」「詳見 D2 衛教單張索引」這些都是行動指引，比空洞的「（待填）」實用。共清 17 處 placeholder（A1 16 處 + B1 1 處）。**版本號決策**：原本算 V3.4.14 延伸不升版，但 V3.4.14 已打包，避免兩個 V3.4.14 ZIP 內容不同的混淆，升 V3.4.15 重新打包。原則「版本號跟 ZIP 一對一」。Python 統一 replace 16 處 + str_replace 1 處，mkdocs --strict 0 warning。**接手第一件事**：Sela 部署實測 (a) A1 院內資源/附錄聯絡資訊/相關文件索引三段都用過渡寫法 (b) B1 預設密碼指引清楚。**規則 12+13+QA+placeholder 過渡四個階段全部告一段落**，從這版起進入純內容補充期（等實際截圖、藥物表、完治率定義、NGS 等資料才能動）。
