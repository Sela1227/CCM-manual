...
1. **驗證 V3.4.5 + V3.4.6 內容重排視覺與內容** — 部署後第一件事，確認 (a) F 檔頁首章節架構表是 F1–F5 五段、F4 內部一-十一連續無跳號 (b) A1 目錄是 4 個部分（不再有第三部分軟體上手）(c) A1 第三部分標題是「工作工具箱」（原第四）(d) B1 目錄是 18 章 + 附錄 + 癌症中心專用軟體 (e) B1 第 2/16/18 章是新加的內容（從 A1 搬來）(f) 沒有跨檔斷連結（前面 grep 確認過只有 index.md 連 A1/B1 根 URL）
2. **A1 第二部分 vs A2 培訓計畫書是否重複（V3.4.5/6 沒處理）** — Sela 已標為「下一階段」。A1 line 464-685 「新人養成指引」（學習路線圖、四階段培訓、checklist）跟 A2 主題正面對撞——A2 整本就是「培訓計畫書」。需先讀 A2 完整結構才能判斷。**等 Sela 主動提**
3. **A1 附錄是否要動（V3.4.5/6 沒處理）** — Sela 已標為「下一階段」。A1 line 1217-1247（重新編號後位置仍在末尾）「附錄」含「重要聯絡資訊」「相關文件索引」，內容是 placeholder（待填）。等 Sela 主動提
4. **驗證 V3.4.4 手機底部導覽 6 個鈕視覺** — 部署後第一件事，確認 (a) 6 個鈕在 360px 寬手機都能塞下圖示+標籤、無被切字 (b) 各鈕 active 狀態正確（在對應頁時該鈕反白）(c) 在較窄手機（< 360px）會不會擠到變形——若是，CSS 微調 font-size 0.7rem → 0.65rem
5. 處理「目錄／目錄」重複（H2 章節 + Material 自動 TOC 撞名）— V3.4.6 後 A1 還有 `## 目錄`、B1 也有 `## 目錄`，兩者都是 H2，跟 Material 自動 TOC 撞名。考慮砍掉 H2 區塊
6. 補 B1 各 HIS 子系統的操作截圖（V3.4.6 後 B1 集中為單一系統手冊，新人最需要）
7. **F 檔巢狀化（V3.4.5 重排後遺留任務，尚未處理）** — V3.4.5 已把 F 檔內容從 F1/F2 兩段重排為 F1–F5 五段，但**仍是單檔頂層、用 H2 區分**。是否要拆 5 檔做成巢狀（仿 A/B/C/H）尚未評估。決策依據：(a) Sela 部署 V3.4.5 體驗後覺得 5 段 H2 是否好用 (b) sidebar 多 5 個條目擴張的高度可不可以接受 (c) 若拆檔，每段獨立成 docs/F1_lab.md 等 5 個檔，工程量類似 V3.4.5 的反向操作。**優先序低**，等 Sela 主動提才動
8. 抗癌藥物速查若有新版，整個 `docs/drug-lookup/` 資料夾原地替換 + 改 H3 版本說明（規則 9）
9. 子宮頸癌 / 子宮體癌 / 卵巢癌完治率定義補入 G_quality-index.md（待各團隊討論）
10. **logo.png（首頁大 logo 600px）是否還需要？** V3.4.1 起所有引用都移除了，但檔案還留在 docs/assets/。下次清理時可考慮砍——但不急，留著不會壞 build；規則 11 仍要求三檔在但 logo.png 沒被引用，必要時可從規則 11 移除這條
11. NGS 內容後續更新時注意：健保給付癌別會擴增（目前 19 種是 113 年 5 月版），補入時記得同步改 F2 NGS 段表格 + 註明版本日期（V3.4.5 起 NGS 在 F2「分子與病理檢測」）

---

## 八、一句話總結

V3.4.6：**純內容版本**，沒動框架、CSS、JS（除版本號註解）。把 A1/B1 重複嚴重的問題用方案 C（最徹底）解掉——A1 變成「**個管師工作流程**」純粹版（砍第三部分軟體上手 267 行，含內部自重複「軟體問題找誰」）；B1 變成「**個管師會用到的所有系統與工具**」（從 A1 搬入帳號申請、LINE Email 雲端工具、遇到問題找誰三個新章節，原 15 章編號全部+1 或+2 變 18 章）。A1 line 1247→983（砍 264 行），B1 line 570→625（增 55 行）。Python 切片＋拼接重組（仿 V3.4.5 F 檔做法），mkdocs --strict 0 warning。踩了一個小坑：B1 新章節原本叫「LINE / Email / 雲端工具」但 ` / ` 在 slugify 下變雙 dash 對不上目錄連結，改用空格分隔解掉。CSS 仍維持 V3.2.0 乾淨基底（規則 8 不破）。**接手第一件事**：Sela 部署實測 (a) A1 是 4 個部分（不再有軟體上手）(b) B1 是 18 章 + 附錄 + 癌症中心專用軟體 (c) B1 第 2、16、18 章新內容（帳號 / 工具 / 求救）正常 (d) Sela 提到的下一階段任務（A1 vs A2 重複、A1 附錄）暫不動，等她主動提。
