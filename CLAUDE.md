...
| V1.9.1 | B/H 重疊整合：B1 移除「癌症中心專屬系統」重複內容（改為 → H 導引），B 改名「HIS 與通用工具」，H 改名「癌症中心專用軟體」，定位清晰不重疊 |
...
V1.9.1 B/H 重疊解決：B1 移除癌症中心系統重複章節改為導引，B 定位為「醫院 HIS + 通用工具」，H 定位為「癌症中心自建專用軟體」。
