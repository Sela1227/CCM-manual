...
1. **驗證 V3.4.17 醫師視角修補** — 部署後第一件事，確認 (a) C3-C6 4 癌別都是 8 段架構 (b) C1/C2 開頭有「⚠️ 準則引用」段 (c) H3 版本資訊統一寫法、不再有重複的「目前版本：V3.6」(d) G 完治率定義之前有「完治率定義邏輯說明」段 (e) F4 給藥概念之前有「根治性 vs 緩和性」提示

**等醫師團隊提供（V3.4.17 衍生）**

2. **C1 肺癌完整內容**——15 處 placeholder（流行病學排名、本院新案數、好發年齡、衛教單張、團隊分機、外部資源等）
3. **C3-C6 八段內容填寫**——4 癌別 × 6 段（一疾病概述 / 二診斷與分期 / 三治療指引 / 六個管照護 / 七常見問題 / 八衛教資源）需要醫師團隊提供
4. **F 系列治療概念引用**——F4 治療策略需指定 NCCN / ESMO / 台灣相關醫學會準則版本
5. **H2 MDT 系統「個案資料最少必填欄位」**——需 H2 系統開發團隊 + 癌症中心主任決定
...
9. 子宮頸癌 / 子宮體癌 / 卵巢癌完治率定義補入 G_quality-index.md
10. NGS 內容後續更新（同步改 F2_pathology.md NGS 段表格）
...
> 結構工程（規則 12+13）+ 規則 14 一致性 + 三輪 QA（新人/老手/醫師視角）告一段落。**剩下大部分是「等醫師團隊或 IT 團隊提供資料」**——文件框架已就位、版面已準備好

---

## 八、一句話總結

V3.4.17：**醫師視角試用發現 7 個問題、修 5 個**。醫師看系統的角度跟個管師完全不同——「這份文件能不能拿給新人看？」「治療建議引用什麼準則？」「品質指標分母為什麼這樣定？」**最嚴重發現**：C3-C6（4 癌別共 238 行）只有「特殊注意事項」+「品質指標」兩段、其他 6 段都標「待補充架構」——醫師看了會覺得這份文件不能拿給新人。**Sela 選「動所有能動的」**：(1) C3-C6 八段架構完整展開，跟 C1/C2 一致（452 行的待填欄位當未來版面）(2) C1/C2 加「⚠️ 準則引用」待補段（建議引用：台灣肺癌學會/NCCN Lung、台灣乳房醫學會/NCCN Breast/ESMO Breast）(3) H3 版本資訊統一寫法（原本「115 年 4 月版」+「目前版本 V3.6」兩處 → 合併一處）(4) G 完治率加「定義邏輯說明」段，回答醫師會問的 4 個 FAQ（「為什麼 Stage IV 排除？」「為什麼治療中扣除？」「為什麼乳癌備註多？」「為什麼服滿半年算完治？」）(5) F4 化療表加 admonition「以下是根治性化療，緩和性會減量/持續/暫停」。**踩到的坑**：第一輪 Python 腳本在 `/home/claude` 執行（沒 cd 到 work），第一件事失敗就 abort，但 build 看起來過了（前次 site/ 殘留）。教訓：**動完一定要逐項 grep 驗證，不能只看 build 過**。**剩 4 個等醫師團隊**（C1/C3-C6 大量臨床細節、F 治療準則引用版本、H2 個案資料必填規範），等下次醫師會議時 Sela 拿這份清單問醫師。**接手第一件事**：Sela 部署實測 (a) C3-C6 4 癌別 × 8 段 (b) C1/C2 準則引用段可見 (c) G 完治率邏輯 4 FAQ (d) F4 根治 vs 緩和提示。
