...
1. **驗證 V3.4.11 F 檔拆檔視覺** — 部署後第一件事，確認 (a) sidebar「F 臨床知識庫」展開後看到 6 個項目（F. 概覽 + F1-F5）(b) F 概覽頁是導覽表、5 子檔連結可點擊跳轉 (c) 各子檔頁首 H1 是「F1. 檢驗判讀」「F2. 分子與病理檢測」等 (d) F4 子檔內容完整（907 行最大、需確認治療方式一-八/局部介入/精準設備/藥物簡介都在）
2. **D 檔合併兩版重複 + 拆檔（規則 13 遺留 + 結構治療）** — 這個是兩步合一，工程比 F 大：(a) 先合併 D2/D3/D4 各自的兩版，逐項比對保留獨家內容 (b) 再拆 D1-D4 四子檔做巢狀。建議下次動之前先寫「逐項決策表」確認合併方向
3. **驗證 V3.4.10 A2 重整視覺**（如未實測）— A2 4 章 + 附錄結構、cross-ref 段引導
4. **驗證 V3.4.9 nav 整合視覺**（如未實測）— sidebar「軟體與工具」大組 6 檔並列
...
10. **A2 跟 A1 是否要 nav 分組**？V3.4.10 後 A2 定位為制度文件、A1 為工作手冊，但 nav 仍在「A 新人通則」之下並列
11. **A2 cross-ref 連結 markdown 化**：目前 cross-ref 用粗體文字，考慮改成可點擊連結。下次動 A2 時順手做

---

## 八、一句話總結

V3.4.11：**規則 13 拆檔第一輪**。F 檔（1302 行）拆 5 個獨立子檔做巢狀——F1 檢驗判讀（43 行）/ F2 分子與病理檢測（178）/ F3 影像判讀（124）/ F4 癌症治療（907）/ F5 個管師追蹤速查（34）；F 主檔變導覽頁（30 行），保留章節架構表並改成 markdown 連結指向 5 子檔。mkdocs.yml nav 加 F 巢狀組。**D 檔盤點時發現嚴重問題**：D2/D3/D4 各出現兩次（line 646-1108 整理版 + line 1109-末尾舊版），兩版內容互有獨家不能簡單砍一個——需先做「結構治療」合併兩版才能拆檔。D 檔列下版候選 #2、待寫合併方案。**踩到的坑**：F 拆檔腳本判斷 H2→H1 條件用 `ln[3].isdigit()` 錯了（`## F1.` 第 3 字是 `F` 不是 `1`），第一輪 5 子檔都沒 H1，sed 替換解掉。**規則 13 拆檔的隱藏前提**：拆檔前要先 grep 看 H 結構健康——多 H1/編號重複/章節重複的檔不能直接拆。**接手第一件事**：Sela 部署實測 F 子檔結構與導覽頁可點擊；如果 F4（907 行）使用後覺得仍偏大，再評估是否拆 F4 內部 4 主題（治療方式 / 局部介入 / 精準設備 / 藥物簡介）為孫檔——但傾向不拆，因為「拆檔」跟「TOC 內部導航」是不同層級的事。
