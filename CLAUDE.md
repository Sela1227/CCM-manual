...
1. **驗證 V3.4.18 內容修補** — 部署後第一件事，確認 (a) C3-C6 4 癌別都有完整 8 段內容（非「待補充」）(b) 所有檔內無「（待填）」字串 (c) A1 課程表有「自學參考」欄、6 週都有 cross-ref (d) A1 結業測驗段有題型/範圍/說明
2. **C3-C6 內容請醫師團隊審閱** — V3.4.18 補的內容明確標「⚠️ 待醫師團隊確認」。優先請醫師審：(a) C3 治療指引（FOLFOX/CapeOx 細節、健保 2025/6 mCRC 免疫治療給付）(b) C4 BCLC 分期建議（特別是 B/C 期治療策略）(c) C5 CCRT 細節 + 鼻咽癌 EBV DNA 流程 (d) C6 NCCN 風險分層 + ADT 監測

**等醫師團隊提供**

3. C1 肺癌剩餘 placeholder（流行病學、好發年齡——這幾個非過渡寫法能解的，是真的等資料）
4. F 系列治療概念引用準則版本
5. H2 MDT 系統「個案資料最少必填欄位」

**老手視角剩餘**

6. 電話追蹤內容三處整合（D3/F5/A1，規則 14 待套用）

**等資料類**

7. 補 B1 各 HIS 子系統的操作截圖
8. 抗癌藥物速查若有新版
9. 婦科完治率定義
10. NGS 內容後續更新
...
> 結構工程 + 三輪 QA + 內容深度補充階段告一段落。**4 癌別文件已從「空殼」變為「待審稿」**，下次主要工作是醫師審閱回饋

---

## 八、一句話總結

V3.4.18：**新人 28 天時間軸試用 + C3-C6 補臨床知識**。Sela 提議「用新人角度模擬上班四週試用系統」，跑 W-2/W-1/W1/W2 共 28 天時間軸發現：(a) **V3.4.15 placeholder 過渡寫法漏網 17 處**散在 B1/B2/C1/E（V3.4.15 grep 範圍只看 A1+B1）→ 全清 (b) A1 第一階段結業測驗沒寫範圍 → 加題型/範圍/說明 (c) A1 課程表沒指向自學參考 → 加 6 週 cross-ref（B1/H1/H2/C0/F2/F4/H3/A2/D1/F1/D3）(d) **C3-C6 V3.4.17 八段架構是空殼**——Sela 同意「以查得到的公開準則先補上去、再等醫師確認」。**主菜**：依國健署 2024、台灣癌症基金會、健保署 2025、NCCN 2025、BCLC 2022 update、中山醫大攝護腺指引等公開來源，給 4 癌別補實質醫學內容（流行病學數據/分期/具體治療藥物 FOLFOX-CapeOx-Atezo+Bev-ADT-Cetuximab/個管任務/4-5 個 Q&A 每癌別）。**每癌別頂標明「待醫師團隊確認」**——責任邊界清楚。從 238 → 927 行（4 倍）。**踩到的坑**：placeholder 漏網的根因是 V3.4.15 grep 範圍限定。**規則層級啟示**：以後遇到「等醫師團隊提供」內容，先用公開準則補 0.x 版本標明來源 + 待審，比留空殼有用。**接手第一件事**：Sela 部署實測 + 排時間請醫師團隊審閱 C3-C6 內容（候選 #2 列出優先審閱重點）。**下次工作主軸從「框架建置」轉為「醫師審稿回饋整合」**。
