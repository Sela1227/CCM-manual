...
1. **驗證 V3.4.12 D 檔合併拆檔視覺** — 部署後第一件事，確認 (a) sidebar「D 表單與範本」展開後看到 5 個項目（D. 概覽 + D1-D4）(b) D 概覽頁是導覽表、4 子檔連結可點擊跳轉 (c) D2 衛教索引內含第二版 5 項獨家（白血球低下警訊 / 頭頸放療 / 骨盆放療 / 手術 / 腸造口）(d) D3 開頭有「通用開場與結尾」段、各情境腳本不變 (e) D4 末尾有「個案追蹤記錄格式」段
2. **驗證 V3.4.11 F 檔拆檔視覺**（如未實測）— sidebar「F 臨床知識庫」展開、5 子檔可點擊
3. **驗證 V3.4.10 A2 重整視覺**（如未實測）— A2 4 章 + 附錄結構
4. **驗證 V3.4.9 nav 整合視覺**（如未實測）— sidebar「軟體與工具」大組
5. 補 B1 各 HIS 子系統的操作截圖
6. **A2 跟 A1 是否要 nav 分組（V3.4.10 衍生）** — A2 是制度文件、A1 是工作手冊。要不要分成「A 新人手冊 / B 制度文件」？等 Sela 部署後決定
7. **A2 cross-ref 連結 markdown 化（V3.4.10 衍生）** — 目前 cross-ref 用粗體文字，考慮改成可點擊連結
8. 抗癌藥物速查若有新版（整個 drug-lookup/ 替換 + 改 H3 版本說明，規則 9）
9. 子宮頸癌 / 子宮體癌 / 卵巢癌完治率定義補入 G_quality-index.md（V3.4.8 重整後在「個案管理三大追蹤指標 → 各癌別完治率定義」H4 之下，待團隊討論）
10. **logo.png（首頁大 logo 600px）是否還需要？** V3.4.1 起所有引用都移除了
11. NGS 內容後續更新時注意：健保給付癌別會擴增（目前 19 種是 113 年 5 月版），補入時記得同步改 F2_pathology.md NGS 段表格

---

## 八、一句話總結

V3.4.12：**規則 13 拆檔第二輪，全部完成**。D 檔（1479 行）的問題比 F 檔複雜——D2/D3/D4 各出現兩次內容互有獨家不能砍。採保守合併策略：以第一版為主（已有 H2+H3 結構、跟 D1 一致）+ 第二版獨家項目補入（D2 補 5 項治療別/症狀別衛教、D3 補通用開場與結尾、D4 補個案追蹤記錄格式）+ 第二版整段砍。然後拆 4 個獨立子檔做巢狀：D1 評估量表彙整（633 行）/ D2 衛教單張索引（59）/ D3 電話追蹤腳本（302）/ D4 書信與轉介範本（181），主檔變導覽頁（27 行）。mkdocs.yml nav 加 D 巢狀組。**規則 13 全部完成**：A/B/C/H/D/F 都已巢狀化，E（525 行）和 G（470 行）保持單檔頂層、符合規則 13 門檻。Python 切片 + 錨點插入，mkdocs --strict 0 warning，跨檔連結無斷。**接手第一件事**：Sela 部署實測 (a) D 子檔結構正確 (b) 第二版獨家 5 項（D2 衛教）+ 通用開場（D3）+ 追蹤記錄格式（D4）都搬入。下一階段建議動 V3.4.10 衍生候選（A1/A2 nav 分組、A2 cross-ref markdown 化），這兩個都是「使用體驗微調」工程小。
