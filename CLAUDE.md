...
| V3.4.12 | **規則 13 拆檔第二輪**：D 檔（1479 行）合併兩版重複（D2 補白血球低下/頭頸/骨盆/手術/腸造口、D3 補通用開場、D4 補追蹤記錄格式）+ 拆 4 子檔做巢狀（D1 評估量表 / D2 衛教 / D3 電話 / D4 書信），主檔變導覽頁。規則 13 全部完成（坑 #36）|
| V3.4.13 | **A2 體驗微調**：nav 加副標「（制度文件）」標明 A2 定位（方案 Y，最小改動）+ cross-ref 9 處 markdown 化（從粗體文字改成可點擊連結 `[A1 工作指導手冊](A_work-guide.md)` 等）（坑 #37）|
| V3.4.14 | **新人視角 QA 修補**：從新進個管師角度試用發現 10 個問題、修 6 個——首頁架構表更新（B+H 合併）、F 子檔 5 個引言重複砍除、A1「個管師是什麼角色？」+「什麼時候要轉介？」H4 升 H3、D1/D4 重複 `---` 砍除、B1 補 H 系列段（坑 #38）|
| V3.4.15 | **placeholder 過渡寫法**：清 A1+B1 共 17 處「（待填）」改成行動指引（「請洽該單位」「報到時主管提供」「詳見 H1/D2」等）（坑 #39）|
| **V3.4.16** | **老手視角資料一致性修補**：個管師分工 4 套版本不一致（A1/H2/H3/userMemories）→ 統一為 A1 權威版本，H2 真名改 A/B/C 匿名，H3 加 cross-ref。MDT 4/7 號改名（消化道癌、肝膽胰癌）+ 加「同日連續開會」說明。**訂規則 14 single source of truth**（坑 #40）|
...
1. **驗證 V3.4.16 個管師分工統一** — 部署後第一件事，確認 (a) A1 第一部分分工表是 A=消化/血液/婦癌、B=肝膽胰/泌尿/頭頸、C=乳/肺/食道/其他 (b) A1 MDT 列表 4 號是「消化道癌團隊（大腸/直腸/胃）」、7 號是「肝膽胰癌團隊（肝/膽/胰）」、有「同日連續開會」說明 (c) H3「支援的癌別」表跟 A1 一致、有 cross-ref 連到 A1 (d) H2「第一次進入系統」是「個管師 A/B/C」（不是真名）、有 cross-ref 連到 A1 (e) 林伯儒醫師在 F/H1/H2/H3/H4/C0 文件資訊「維護單位」保留、不影響
2. **老手視角剩餘問題（V3.4.16 沒處理）**：
   - **2a 高頻動作「電話追蹤」內容散三處**（D3/F5/A1 第三部分）—— 規則 14 該指定權威源（建議 D3，最完整），其他用 cross-ref。需先確認 D3/F5/A1 三處內容差異後再合併
   - **2b H3 系統版本資訊不一致** —— H3 line 27「（115 年 4 月 23 日版）」+ line 53「目前版本 V3.6」並存。要統一寫法
3. 驗證 V3.4.15 placeholder 過渡寫法（如未實測）
4. 驗證 V3.4.10-V3.4.14 各版本（如未實測）

**等資料類**

5. 補 B1 各 HIS 子系統的操作截圖
6. 抗癌藥物速查若有新版（整個 drug-lookup/ 替換 + 改 H3 版本說明，規則 9）
7. 子宮頸癌 / 子宮體癌 / 卵巢癌完治率定義補入 G_quality-index.md
8. NGS 內容後續更新（同步改 F2_pathology.md NGS 段表格）

**雜項**

9. **logo.png（首頁大 logo 600px）是否還需要？**
10. **再做一次新人/老手視角試用** — 實際資料填入後可再做一次 QA、找新一輪問題

> 結構工程（規則 12+13）+ 新人 QA + placeholder 過渡 + 老手 QA 階段告一段落。**規則 14（資料一致性）已訂下、首次套用個管師分工**，規則 14 待套用的還有「電話追蹤內容散三處」（候選 #2a）

---

## 八、一句話總結

V3.4.16：**老手視角資料一致性修補**。Sela 提議「老手視角試用」，跑 5 個高頻情境發現**個管師分工有 4 套不一致版本**（A1/H2/H3/userMemories），其中 H2 用了真名「楊靜雯/郭美伶/林伯儒」+ 把醫師當成個管師。Sela 確認真實情況：「林伯儒主任暫代缺人的第三個管師」，決定全用「個管師 A/B/C」匿名寫法（人事異動不會讓文件過期）。同時釐清 MDT 結構：胃癌跟大腸直腸是**同一個** MDT 團隊（消化道癌 MDT），但跟肝膽胰 MDT **同一天連續開**（不是同一場）。三檔修整：A1（權威源）+ H3（按新分工 + cross-ref）+ H2（真名→A/B/C + cross-ref）。**訂規則 14 single source of truth**——個管師分工/MDT 編號/量表/指標/藥物給付等跨檔重複資料，指定權威源，其他檔用 cross-ref 不複製。**接手第一件事**：Sela 部署實測 (a) A1/H2/H3 三處分工一致 (b) MDT 4 號是消化道癌、7 號是肝膽胰癌、有同日說明 (c) H2 真名都改 A/B/C。**老手視角剩 2 個未修問題**列候選 #2a/2b：電話追蹤內容散三處（D3/F5/A1）需指定權威源、H3 版本資訊兩種寫法並存待統一。
