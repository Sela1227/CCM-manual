...
**對話脈絡**：V3.0.0 從零重做，把 V2.x 的 17 份巢狀分類 MD 收斂成 4 篇平鋪主檔（工作 / 養成 / 工具 / 指標），架構上消除 V2.x 的 sidebar 標題重複問題。V3.0.1 主色換 #25506b、sidebar 永久顯示。
...
**#18（V3.0.1）palette: custom 不會自動讀任何顏色**
- 症狀：mkdocs.yml 把 `primary: indigo` 改成 `primary: custom` 後，header 變成空白沒主色
- 原因：Material 的 custom palette **必須**在 CSS 用 `[data-md-color-primary="custom"]` 選擇器設 CSS 變數，否則沒值
- 做法：extra.css 加：
  ```css
  ...
  ```
- 注意：深色主色（如 #25506b）保留預設白字即可；淺色主色（如 #C9C8C7）才需要額外 override `--md-primary-bg-color` 為深色
...
| **V3.0.1** | 主色 #25506b（custom palette，坑 #18）、sidebar 永久顯示沿用 V2.0.2 那 4 條核心規則（V3 不需要 V2.0.3 的 5–9 補丁） |
...
1. **驗證 V3.0.1 sidebar 永久顯示在 MacBook Air（885px viewport）正常** — Sela 部署後第一件事，需要截圖確認 (a) 左邊 sidebar 跟主內容並排不浮動 (b) 主色 #25506b 視覺對 (c) 手機底部 4 鍵正常
...
V3.0.1：主色換成 #25506b 深藍灰（custom palette，坑 #18），sidebar 永久顯示用 V2.0.2 那 4 條已驗證的核心規則（V3 平鋪 nav 不需要 V2.0.3 的 5–9 補丁）。連續 7 版（V2.0.x → V3.0.0 → V3.0.1）打地鼠的「sidebar 標題重複」問題從架構上消除，下版可以開始做內容（HIS 截圖、藥物速查更新），不必再動框架 CSS。
