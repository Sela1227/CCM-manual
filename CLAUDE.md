...
| V3.4.16 | **老手視角資料一致性修補**：個管師分工 4 套版本不一致（A1/H2/H3/userMemories）→ 統一為 A1 權威版本，H2 真名改 A/B/C 匿名，H3 加 cross-ref。MDT 4/7 號改名 + 加「同日連續開會」說明。**訂規則 14 single source of truth**（坑 #40）|
...
| **V3.4.20** | **logo.png 砍除 + 第四輪 QA**：砍 logo.png（V3.4.1 起無引用、244 KB）規則 11 簡化為 2 logo 檔；第四輪 QA 三視角組合試用，發現 1 個必修問題（首頁「各癌別照護指引」卡片寫死跳 C1，改連 C0 通論 + 卡片描述更新）+ 2 個合理觀察。前三輪 QA 累積修補品質高（坑 #44）|
...
1. **驗證 V3.4.20** — 部署後第一件事，確認 (a) 首頁「各癌別照護指引」卡片連到 C0_general/（不再是 C1_lung-cancer）+ 描述更新 (b) `docs/assets/` 只有 favicon.png + logo-header.png 兩個 logo 檔（無 logo.png）

**最重要的下一步（從 V3.4.18 起就一直是優先順位 #1）**

2. **C7-C13 七個新檔請醫師團隊審閱**（V3.4.19 補的）—— 優先審：C7 食道癌健保 2024/4 二線免疫給付、C8 胃癌 HER2/MSI/PD-L1 三標記分流、C9 肝膽胰 Whipple + mFOLFIRINOX、C10 泌尿 BCG 灌注 + 尿路重建、C11 婦癌 BRCA + PARP、C12 血液淋巴 CAR-T + Stupp、C13 其他癌通用原則

3. **C3-C6（V3.4.18 已補的）也要醫師審**：mCRC 免疫健保 2025/6 給付、BCLC 分期、CCRT、攝護腺風險分層
...
7. 電話追蹤內容三處整合（D3/F5/A1，規則 14 待套用）

**等資料類**

8. 補 B1 各 HIS 子系統的操作截圖
9. 抗癌藥物速查若有新版（drug-lookup/ 替換 + 改 H3 版本說明，規則 9）
10. NGS 內容後續更新（同步改 F2_pathology.md）

**雜項**

11. 再做一次 QA 試用（醫師審稿回饋整合後）

> **結構工程 + 三輪 QA + placeholder 過渡 + C 系列大擴張 + 第四輪 QA + logo 清理全部完成**。剩下純粹是「等醫師審稿 / 等實際資料」階段。下一個版本應由醫師回饋觸發、不該由 Sela 主動發起結構/內容變動

---

## 八、一句話總結

V3.4.20：**logo.png 砍除 + 第四輪 QA 試用**。Sela 列出兩件候選工作：(1) logo.png 是否還需要 (2) 再做一次 QA 試用。同版做完。**logo.png**：V3.4.1 起所有引用都移除、grep 全 docs 無引用、244 KB 直接 rm，build 0 warning、規則 11 從 3 logo 檔簡化為 2 logo 檔。**第四輪 QA**：用三視角組合（新人速查 + 老手日常 + 醫師審視）跑核心情境，發現 1 個必修——首頁「各癌別照護指引」卡片寫死跳 C1_lung-cancer，V3.4.19 新增 14 個 C 系列檔後沒更新。三個個管師負責不同癌別，從首頁直接進肺癌對 A/B 個管師不合理。修法：改連 **C0 癌症照護通論**（所有人都會看的入口）+ 卡片描述從「肺癌、乳癌及其他癌別」改成「從通論到 12 個癌別專屬照護指引」。**2 個觀察記錄不修**：sidebar C 區段 14 項較長（合理，10 MDT 癌別必然結果）、26 個「待補充」殘留（合理，等實際資料）。**規則 1/8/9/11/12/13/14 全部通過**。**踩到的小坑**：grep 統計 C13「待醫師確認」= 0 以為漏網，閱讀內容才知 C13 因「其他癌」異質性用「待醫師團隊**提供**」而非「**確認**」，這是有意的用詞差異——教訓 grep 統計不能取代閱讀理解。**第四輪 QA 結論**：V3.4.16-V3.4.19 連續四版內容大幅變動，本輪 QA 只發現 1 必修 + 2 觀察——前三輪 QA 累積品質高、無退化。**接手第一件事**：Sela 部署實測首頁卡片連 C0、logo 檔只剩兩個。**下一個版本應由醫師審稿回饋觸發**——不該由 Sela 主動發起結構/內容變動，因為文件框架已就位、內容草稿（依公開準則整理）已標明「待醫師確認」、剩下都是醫師團隊與 IT 團隊的事。
