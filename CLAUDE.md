...
1. **驗證 V4.0.1 去重落實** — **本版重點交付物的可用性確認**。檢查 (a) A1 第 4 欄是「✓/ⓘ」狀態而非時間 (b) C0「六、五癌篩檢」段精簡為引言 + D2 cross-ref (c) A3「個管師團隊」段無「負責癌別」欄 (d) C1/C2「四、品質指標」改名「個管師日常追蹤重點」(e) G2 details 段是「實際數據+閾值」非完整定義 (f) `mkdocs build --strict` 0 warning

**最重要的下一步：C 系列醫師審稿**

2. **C7-C13 七個新檔請醫師團隊審閱**（V3.4.19 補的）—— 現在有 A3 醫師清單 + G2 實際成績 + C 系列重新定位的「個管師日常追蹤重點」可對應
3. **C3-C6 V3.4.18 已補的內容也要醫師審**
...
7. C1 肺癌剩餘 placeholder
8. F 系列治療概念引用準則版本
9. H2 MDT 系統「個案資料最少必填欄位」

**老手視角剩餘**

10. **電話追蹤內容三處整合（D3/F5/A1，規則 14 待套用）**

**等資料類**

11. 補 B1 各 HIS 子系統的操作截圖
12. 抗癌藥物速查若有新版
13. NGS 內容後續更新
14. **時效性內容年度檢視**（規則 15）：每年檢視 EGFR / 五癌篩檢 / 健保藥物 / **G2 替換為 115 年度版**
15. 五癌篩檢推廣 QR code 2027 年改版

**雜項**

16. 美編+UI 第二輪審視（內文檔字級/間距/表格密度等）
17. emoji 用法一致化（觀察 #8，工程大）
18. 再做一次 QA 試用（醫師審稿回饋整合後）

---

## 八、一句話總結

V4.0.1 **使用者視角去重 8 件**：基本款 4 件（A1 第 4 欄簡化、C0 砍 5 癌篩條件表、A3 砍負責癌別欄、A3 同日連續開會只留 1 處）+ 指標 4 件（G2 details 精簡、G 主檔加 cross-ref 欄、C1/C2 改個管師日常追蹤重點、G+G2 快速查表整合分工）。**G/G2/C 三者明確分工**：G 主檔=指標定義、G2=本年成績、C 系列=個管師日常追蹤重點。**下版第一優先**：驗證 V4.0.1 去重落實。
