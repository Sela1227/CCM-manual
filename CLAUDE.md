...
| **V3.4.26** | **補入肺癌 EGFR 基因檢測補助專案資料**：Sela 上傳 PDF（台癌基金會專案，Ib/II/IIIa 期肺癌 / 最高 12,000 元 / 2026/12/31 截止）→ 整合到 4 處（C1 外部資源段為主寫入點 + C1 FAQ + F2 + E 經濟困難段）+ 規則 14 套用（C1 為單一資料源、其他 cross-ref）+ 標明時效避免過期誤導（坑 #50）|
...
1. **驗證 V3.4.26 EGFR 補助專案資料** — 部署後第一件事，確認 (a) C1 外部資源段有完整 EGFR 補助專案（補助對象/金額/期間/6 項文件/送件方式/許雅涵分機/工作流程 5 步）(b) C1 FAQ「病人猶豫做基因檢測」有 cross-ref 到 EGFR 段 (c) F2 末尾有「💰 早期肺癌 EGFR 補助」cross-ref C1 (d) E 第四段「可用資源」表有兩行（一般經濟補助 + EGFR 補助專案）

**剩餘 placeholder（A3 + 環境章節同批詢問主管）**

2. **A3 剩餘 placeholder 填入**（V3.4.25 後）：38 醫師分機（召集人 8 位優先）/ 共通支援單位 / 個管師真名 / 行政秘書 / 「人文會議室 vs 人文藝術館會議室」命名
3. **V3.4.23 環境章節 placeholder**：5 工作空間樓層、14 跨科單位分機、員工餐廳營業時段、服儀規範
...
11. 抗癌藥物速查若有新版
12. NGS 內容後續更新
13. **補助專案類資料更新**：2026/12/31 EGFR 補助結束後是否續辦、其他癌別有無類似補助（婦癌 BRCA、大腸 RAS 等）

**雜項**

14. 美編+UI 第二輪審視
15. emoji 用法一致化（觀察 #8，工程大）
16. 再做一次 QA 試用（醫師審稿回饋整合後）

> **EGFR 補助專案 4 處整合完成**——病人從 C1 外部資源、C1 FAQ、F2 NGS 段、E 經濟困難段四個切入點都查得到、且 single source of truth 在 C1（規則 14）

---

## 八、一句話總結

V3.4.26：**補入肺癌 EGFR 基因檢測補助專案資料**。Sela 上傳 PDF（台灣癌症基金會專案：Ib/II/IIIa 期肺癌 / 上限 12,000 元 / 107 名 / 2026/12/31 截止 / 經濟弱勢優先）。**思考最佳放置位置**：這份資料 4 個候選位置（C1 外部資源 / F2 NGS / D2 衛教 / E 經濟困難）。**做法**：(1) **C1 外部資源段**為主要寫入點——完整補助對象/金額/名額/期間/6 項申請文件/Email+郵寄送件方式/許雅涵分機/個管師 5 步工作流程 + ⚠️ 時效提醒 (2) C1 FAQ「病人猶豫做基因檢測」加 cross-ref (3) F2 末尾加「💰 早期肺癌 EGFR 補助」短段 + cross-ref C1 (4) E 第四段「可用資源」表升級——原本「台灣癌症基金會 — 經濟補助」太籠統、拆兩行（一般經濟補助 + EGFR 補助專案）。**規則 14 套用**：補助細節 single source of truth 放 C1，其他三處 cross-ref 同一個 anchor `#台灣癌症基金會肺癌-egfr-基因檢測補助專案`——未來補助條件變動只需改 C1 一處。**踩到的小坑**：第一輪修 4 在 E 末尾加新段、但 E 第四段已有「可用資源」表 → 重複寫成兩處 → 發現後砍末尾段、合併到現有表升級。**教訓**：**寫入既有檔前先 grep 看現況**，不要憑想像寫新段、要先看「現有結構是否已有合適位置」。**時效處理**：所有出現補助專案的位置都標 2026/12/31 截止日 + ⚠️ 「請於申請前以台癌官網為準」避免 2027 後過時誤導。**接手第一件事**：Sela 部署實測 4 處 cross-ref 通暢 + 補助專案資料完整 + 順便填 A3 + 環境章節剩餘 placeholder。**下一階段**：C7-C13 醫師審稿仍是優先順位 #1（現在有 A3 醫師清單對應）。
