...
| V3.4.17 | **醫師視角內容深度修補**：C3-C6 八段架構展開（從 238→452 行）+ C1/C2 加準則引用待補段 + H3/F2 版本資訊統一 + G 完治率定義邏輯說明（4 個 FAQ）+ F4 化療根治 vs 緩和區分提示。剩 4 個等醫師團隊（坑 #41）|
...
| V3.4.19 | **C 系列癌別涵蓋不全 → 補 7 個新檔（方案 A）**：新人 28 天 V3.4.18 後重跑發現結構破口（A1 分工表寫 10 癌別但 C 系列只覆蓋 6 癌別）→ 拆 C3-C6 為 4 獨立檔 + 新建 C7-C13 七個（食道/胃/肝膽胰/泌尿/婦癌/血液淋巴/其他癌）**全部依公開準則整理 + 標明「待醫師團隊確認」**。C 系列從 6 → 12 癌別+通論。順便修 A1 第一部分多餘空白行（坑 #43）|
...
| **V3.4.21** | **PM 視角審視 7 件修補**：(1) A1 個管師分工表加 markdown 連結（13 個 cross-ref，跨越 MDT 編號 vs C 檔編號兩套）(2) B1 HIS 12 子系統加 anchor 連結 (3) C0「C1-C6」→「C1-C13」(4) C1/C2 開頭多餘 `---` 砍除（跟 C3-C13 一致）(5) A1「deeper」→ 純中文 (6) A1 兩個 💡 提示語合併 (7) A1 ECOG 改 cross-ref D1（規則 14 套用第二案例）（坑 #45）|
...
1. **驗證 V3.4.21** — 部署後第一件事，確認 (a) A1 個管師分工表 13 個 markdown 連結可點擊、跳到對應 C 檔 (b) B1 第 1 章 HIS 子系統表「→ §4」「→ §5」等連結可點、跳到該章 (c) C0 引言寫「C1-C13 各節」(d) C1/C2 開頭格式跟 C3-C13 一致（無多餘 `---`）(e) A1 已無「deeper」中英夾雜 (f) A1 兩個 💡 提示語合併為一個 (g) A1 ECOG 段是 cross-ref 不是表格

**最重要的下一步（從 V3.4.18 起就一直是優先順位 #1）**

2. **C7-C13 七個新檔請醫師團隊審閱**（V3.4.19 補的）—— 優先審：C7 食道癌、C8 胃癌、C9 肝膽胰、C10 泌尿、C11 婦癌、C12 血液淋巴、C13 其他癌
3. **C3-C6（V3.4.18 已補的）也要醫師審**

**等醫師團隊**

4. C1 肺癌剩餘 placeholder
5. F 系列治療概念引用準則版本
6. H2 MDT 系統「個案資料最少必填欄位」

**老手視角剩餘**

7. **電話追蹤內容三處整合（D3/F5/A1，規則 14 待套用）** — V3.4.21 已套用 ECOG（A1→D1），下次套用電話追蹤（A1 第三部分→D3 / F5）

**等資料類**

8. 補 B1 各 HIS 子系統的操作截圖
9. 抗癌藥物速查若有新版
10. NGS 內容後續更新

**雜項**

11. 再做一次 QA 試用（醫師審稿回饋整合後 / 或 PM 視角第二輪）

> **PM 視角審視已完成第一輪**——主要動了跨檔跳頁體驗（cross-ref 完整性）。下一個版本應由醫師審稿回饋觸發、或 Sela 主動發起 PM 第二輪審視（其他檔的語句/連結）

---

## 八、一句話總結

V3.4.21：**PM 視角審視 7 件修補**。Sela 切換成資深 PM 角色「逐頁審核語句順不順、按鈕跳頁符不符合流暢度」——跟前四輪 QA 找「漏的、錯的、不一致的」不同，PM 找「讀起來怎樣、用起來順不順」。**最重要發現**：A1 個管師分工表用「MDT 編號」（4=消化道、5=血液...）但 sidebar 的 C 檔是「C 檔編號」（C3 大腸、C8 胃、C12 血液...），新人從 A1 看到「個管師 A 負責 4. 消化道癌」去 sidebar 找 C4 → **C4 是肝癌**！兩套編號各有意義不該統一，但 A1 分工表該幫使用者跨越——加 13 個 markdown 連結 + 註解說明「上表是 C 檔編號、下表是 MDT 編號」。**B1 也類似破口**：12 個 HIS 子系統純文字沒連結、只有 4 個 H 系列軟體有連結——加 12 個 anchor 連結 `[→ §4](#4-門診系統)` 跨越「總覽 → 詳細」斷層。**規則 14 累積第二案例**：A1 ECOG 段改 cross-ref 連 D1（D1 是評估量表權威源）。**順手修**：A1「deeper 觀察」中英夾雜改純中文、A1 兩個 💡 提示框合併、C0「C1-C6 各節」過時改 C1-C13、C1/C2 開頭多餘 `---` 砍除（跟 C3-C13 一致）。**踩到的小坑**：str_replace 改 A1 ECOG 段時 old_str 假設有說明文字、實際是直接接表格——教訓「str_replace 改既有內容前要先看實際格式」。**接手第一件事**：Sela 部署實測 (a) A1 分工表連結可點 (b) B1 HIS 連結可點。**下一階段**：等醫師審稿回饋 / 或 Sela 發起 PM 第二輪審視（這次主要動 A1 + B1 + C0 + C1/C2，其他檔還沒審）。
