...
| V3.4.19 | **C 系列癌別涵蓋不全 → 補 7 個新檔（方案 A）**：拆 C3-C6 為 4 獨立檔 + 新建 C7-C13 七個（食道/胃/肝膽胰/泌尿/婦癌/血液淋巴/其他癌）**全部依公開準則整理 + 標明「待醫師團隊確認」**。C 系列從 6 → 12 癌別+通論（坑 #43）|
...
| **V3.4.23** | **A1 加「認識彰濱秀傳的環境」新章節**：插在「其他常規工作」與「癌症治療基本認識」之間，4 段約 2500 字（會議室/門診辦公室/食/衣），合理範本+ placeholder 標明位置分機待主管提供。新人 D1-D7 報到第一週實用資訊（坑 #47）|
...
1. **驗證 V3.4.23 環境章節** — 部署後第一件事，確認 A1 第一部分有「認識彰濱秀傳的環境」H3 + 4 個 H4（會議室/門診辦公室/食/衣）+ 提示框引導
2. **環境章節 placeholder 填入**（Sela 主管確認後）：
   - 5 個工作空間實際樓層+室號
   - 14 個跨科單位實際分機+樓層
   - 員工餐廳實際營業時段
   - 服儀規範（制服樣式、鞋色等）
...
3. **C7-C13 七個新檔請醫師團隊審閱**（V3.4.19 補的）
4. **C3-C6（V3.4.18 已補的）也要醫師審**

**等醫師團隊**

5. C1 肺癌剩餘 placeholder
6. F 系列治療概念引用準則版本
7. H2 MDT 系統「個案資料最少必填欄位」

**老手視角剩餘**

8. **電話追蹤內容三處整合（D3/F5/A1，規則 14 待套用）**

**等資料類**

9. 補 B1 各 HIS 子系統的操作截圖
10. 抗癌藥物速查若有新版
11. NGS 內容後續更新

**雜項**

12. 美編+UI 第二輪審視（內文檔的字級/間距/表格密度等還沒審）
13. emoji 用法一致化（觀察 #8，工程大）
14. 再做一次 QA 試用

> **環境章節已就位**——D1 報到第一週實用資訊已有框架，等 Sela 跟主管確認 placeholder 細節即可上線

---

## 八、一句話總結

V3.4.23：**A1 加「認識彰濱秀傳的環境」新章節**。Sela 提議「加入一個篇幅，認識彰濱環境，包含所有會議室，各個門診/辦公室，還有生活所需的食衣」——這是新人 D1-D7 報到第一週實際會用到的資訊，前面所有 QA 試用（新人/老手/醫師/PM/美編）都沒碰過這塊（都聚焦在工作流程、系統操作、臨床知識）。**用 ask_user_input_v0 三題對齊方向**：完整 4 段 / Sela 寫合理範本（可改）+ placeholder / 放 A1 第一部分內。**位置選定**：插在「其他常規工作」與「癌症治療基本認識」之間——銜接邏輯是「工作介紹尾 → 環境介紹 → 轉入醫學內容」。**4 段內容（約 2,500 字）**：(1) 主要工作場所與會議室（5 空間）(2) 相關門診與辦公室（14 個跨科單位+分機）(3) 食：院內外餐飲+ MDT 延到 1:30 的實務提示 (4) 衣：服儀+置物+報到第一天必帶清單。每段提供合理範本但位置/分機全標 placeholder「（待填）」+ 開頭強調「實際位置與分機請以報到時主管或學姊帶你走過的版本為準」。**踩到小坑**：grep 統計新章節 H4 = 4 用全檔模式抓到 11 個（A1 全文多處「1. xxx」「2. xxx」H4），改用 awk 範圍限定才正確。**規則 1 修訂落實驗證**：V3.4.23 是 V3.4.22 啟示「歷史註解不寫死版本號」後第一個升版、本次 sed 沒誤動任何歷史註解 ✓。**接手第一件事**：Sela 部署實測 + 跟主管確認後填 placeholder（5 工作空間樓層、14 跨科單位分機、員工餐廳營業時段、服儀規範、報到必帶清單）。**下一階段**：C7-C13 醫師審稿仍是優先順位 #1。
