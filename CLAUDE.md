...
1. **F 檔重排（V3.4.5 候選最大工作）** — Sela V3.4.4 提「臨床知識庫的分項不是很好，整個看過整理，再重排序分配」。提案方案 A：四分
   - **F1 檢驗判讀**：CBC、生化、腫瘤指數、基因/NGS、IHC（從現 F1 抽出）
   - **F2 影像判讀**：影像總覽、各癌別常用影像、報告關鍵詞（從現 F1 影像段獨立——影像不是「檢驗」）
   - **F3 癌症治療**：六種治療方式、手術／化療／放療／標靶／免疫／荷爾蒙、組合、局部介入、院內精準設備（從現 F2 一-八 + 十 + 十一）
   - **F4 個管師追蹤速查**：追蹤問題速查（手術後/化療中/放療中/口服）+ 抗癌藥物副作用速查表（化療 32／標靶 86／免疫 10／荷爾蒙 11，139 種）（從現 F2 第九 + 第十二）
   - 規模：1285 行重排，動到大量內容。坑 #20 警告過合併拆 MD 會丟內容——這次是**拆**不是合併，但仍要謹慎逐段搬，每搬完一段 build 一次驗證
   - 風險：F 檔現在是平鋪頂層，4 分後是否要巢狀？依規則 3 條件評估。或保留單檔頂層、用 H2 章節區分（最安全）
...
7. 評估 D/E/F/G 是否要拆檔（如果拆，可以也巢狀化做成 D1/D2... 等）——F 檔重排（#1）做完再看
...
9. NGS 內容後續更新時注意：健保給付癌別會擴增（目前 19 種是 113 年 5 月版），補入時記得同步改 F 檔表格 + 註明版本日期

---

## 八、一句話總結

V3.4.4：Sela 在 V3.4.3 部署後選了「方案 B（4 → 6 個）但要進階員工的鈕」——這版就是把手機底部導覽從新人導向（首頁／工作 A／HIS B1／指標 G）改成進階員工日常用的 6 個（首頁／個管 H1／MDT H2／藥物 H3／指標 G／知識 F），SVG 圖示對應同步換、CSS 不動因為 `flex: 1` 自動均分。F 檔重排是 V3.4.4 同時提出但**沒在這版動**——已寫成方案 A 放下版候選工作 #1，等 Sela 確認方案再開動（涉及 1285 行重排、坑 #20 教訓需謹慎）。CSS 仍維持 V3.2.0 乾淨基底（規則 8 不破）。**接手第一件事**：Sela 部署實測 6 個鈕在 360px 手機塞得下、active 狀態正確；如果擠到字被切，CSS 微調 font-size 0.7rem → 0.65rem。F 檔重排方案 A 等 Sela 點頭再動。
