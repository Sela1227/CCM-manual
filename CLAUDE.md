...
**#33（V3.4.9）B/H 整合「軟體與工具」一區（方案 A 變體）**
...
1. **驗證 V3.4.9 nav 整合視覺** — 部署後第一件事，確認 (a) sidebar 看到「軟體與工具」大組，6 檔並列（B1/B2/H1-H4）(b) H 系列項目末尾「（癌症中心專用）」字串可見、不過度折行 (c) 原 H 大組消失，nav 頂層只剩 7 組（首頁/A/軟體與工具/C/D/E/F/G 共 8 個入口）(d) 跨檔連結（B1 末尾「癌症中心專用軟體」段、首頁 8 卡片）仍能正常跳轉
2. **驗證 V3.4.8 C3-C6 + G 重整**（如未實測）— C3-C6 是 1+4+12、G 是 1+7+19+11
3. **驗證 V3.4.7 A 檔層級重排**（如未實測）— A 檔 TOC 32 項、5 主題塊收攏
4. **驗證 V3.4.5 + V3.4.6 內容重排**（如未實測）— F 檔 F1–F5、A1 是 4 個部分、B1 是 18 章
5. **驗證 V3.4.4 手機底部導覽 6 鈕視覺**（如未實測）
...
8. **B1 + F 檔的「## 目錄/章節架構」也要砍**（規則 12 第三條）— A 檔已砍、下次動 B1/F 檔時順手
9. **B1 末尾「## 癌症中心專用軟體」段**（V3.4.6 加的）V3.4.9 nav 整合後變多餘，但保留也 OK。下次動 B1 時再評估是否砍
10. 補 B1 各 HIS 子系統的操作截圖（V3.4.6 後 B1 集中為單一系統手冊，新人最需要）
11. **F 檔巢狀化（規則 13 該拆）** — F 檔 1302 行 > 1000 行門檻。**優先序低**，等 Sela 主動提
12. **D 檔巢狀化（規則 13 該拆）** — D 檔 1479 行 > 1000 行門檻
13. 抗癌藥物速查若有新版，整個 `docs/drug-lookup/` 資料夾原地替換 + 改 H3 版本說明（規則 9）
14. 子宮頸癌 / 子宮體癌 / 卵巢癌完治率定義補入 G_quality-index.md（V3.4.8 重整後在「個案管理三大追蹤指標 → 各癌別完治率定義」H4 之下，待團隊討論）
...
V3.4.9：**nav 結構整合**——回應 Sela 觀察「B/H 都是講軟體的」，把原本兩個獨立大組（B HIS 與通用工具 + H 癌症中心專用軟體）合併為「軟體與工具」一個大組，6 檔並列（B1/B2/H1-H4）。H 系列項目名稱**末尾加「（癌症中心專用）」標記**，保留「醫院共用 HIS vs 癌症中心自開發 web app」的分類資訊。**只動 mkdocs.yml**——檔名不變、首頁 8 張卡片連結不變、手機底部導覽 6 個鈕（V3.4.4 改的指向 H1/H2/H3）連結不變、跨檔連結 `[..](H1_xxx.md)` 不變。nav 頂層大組從 8 個變 7 個。Material wide-mode 處理長標題折行 OK，規則 3 巢狀條件全過。mkdocs --strict 0 warning。**接手第一件事**：Sela 部署實測 (a) sidebar 看到「軟體與工具」大組，6 檔並列且 H 系列「（癌症中心專用）」字串可見 (b) 沒有跨檔斷連結 (c) 「（癌症中心專用）」如果 sidebar 折行太醜，下次可調為更短標籤（如「（中心專用）」或「★」）。
