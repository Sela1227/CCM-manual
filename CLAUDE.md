...
1. **驗證 V3.4.2 三項修補視覺** — 部署後第一件事，確認 (a) header 左上角 logo 在 wide-mode（≥1220px）/ tablet（768–1219px）/ mobile（<768px）三個 viewport 都看得到 SELA 橘色 icon，且不會跟漢堡或站名擠在一起 (b) 內文 padding 在 wide / tablet / mobile 三種都比 V3.4.1 寬（1.4/2.5rem）(c) F 檔 NGS 章節新增區塊（19 癌別表、醫院資格三條件、結果上傳義務、對話要點）有正常渲染
...
7. **logo.png（首頁大 logo 600px）是否還需要？** V3.4.1 起所有引用都移除了，但檔案還留在 docs/assets/。下次清理時可考慮砍——但不急，留著不會壞 build；規則 11 仍要求三檔在但 logo.png 沒被引用，必要時可從規則 11 移除這條
8. NGS 內容後續更新時注意：健保給付癌別會擴增（目前 19 種是 113 年 5 月版），補入時記得同步改 F 檔表格 + 註明版本日期

---

## 八、一句話總結

V3.4.2：V3.4.1 部署後 Sela 反映三件事——(1) logo 在 tablet viewport 完全消失（V3.4.1 砍 drawer logo 沒考慮 Material 在 < 1220px 預設也不顯示 header logo，副作用沒抓到，坑 #27）(2) 內文 padding 還是太貼（1/1.5rem 不夠）(3) NGS 章節缺健保給付資訊。這版一次解掉：CSS 強制 `.md-header__button.md-logo { display: inline-flex !important }`、padding 加大到 1.4/2.5rem、F 檔補 19 癌別表 + 醫院資格三條件 + 結果上傳義務（依國健署 NGS 懶人包 PDF）。CSS 仍維持 V3.2.0 乾淨基底（規則 8 不破）。**接手第一件事**：Sela 部署實測 (a) wide / tablet / mobile 三 viewport 都能看到 header 左上角 logo（特別是 tablet 範圍）(b) 內文 padding 比 V3.4.1 寬但沒擠到 TOC (c) F 檔 NGS 新增區塊渲染正常。如果 logo + padding 在某 viewport 翻車，**只 git revert 那條 CSS 規則**，不要擴大改動範圍。
